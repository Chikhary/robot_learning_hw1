...
def maniskill_dataset_transform(trajectory):
    for i in range(0, len(trajectory)):
        ...
        trajectory[i]["action"] = trajectory[i]["action"].numpy() if hasattr(trajectory[i]["action"], 'numpy') else np.asarray(trajectory[i]["action"])
    ...
    def add(self, obs, action, goal, goal_img, language_instruction=None, pose=None, terminal=0, 
        ...
        if self._cfg.dataset.encode_with_t5:
            if language_instruction is not None:
                ...
                language_instructionArray = np.asarray(language_instruction)
                output_length = self._dataset_tmp["t5_language_embedding"][self._index]
                output_length.zero_()
                self._dataset_tmp["t5_language_embedding"][self._index] = torch.tensor(language_instruction[:min(language_instructionArray.shape[0], self._cfg.max_block_size)], dtype=torch.float, device=self._cfg.device)
            else:
