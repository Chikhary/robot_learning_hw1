...
"""
Fonction qui prend en entrée des images dans un tensor de la forme (batch_size, height, width, channels) et une
configuration cfg qui contient la taille des patches et d'autres paramètres du modèle et divise chaque image en patches.
"""
...
"""
Fonction qui applique le calcul du positional embedding classique.
"""
...
"Applique le self-attention du paper 'Attention is all you need'"
class Head(nn.Module):
    ...
    def __init__(self, head_size, n_embd, dropout):
        super().__init__()
        """
        Initialisation des couches linéaires des NLP.
        """
    ...
    def forward(self, x, mask=None):
        ...
        out = wei @ v
        return out
"Applique le multi-head self-attention du paper 'Attention is all you need'"
...
"Applique le feed-forward du paper 'Attention is all you need'"
...
"Applique Transformer Encoder Block du paper 'Attention is all you need'"
...
class GRP(nn.Module):
    def __init__(self, cfg, mlp_ratio=4):
        ...
        self.patch_projection = nn.Linear(cfg.patch_size*cfg.patch_size*3, cfg.n_embd)
        self.goal_patch_projection = nn.Linear(cfg.patch_size*cfg.patch_size*3, cfg.n_embd)
    ...
    def forward(self, images, goals_txt, goal_imgs, targets=None, pose=None, mask_=False):
        ...
        obs_patches = self.patch_projection(obs_patches)
        patches_g = self.goal_patch_projection(patches_g)

        # Map the vector corresponding to each patch to the hidden size dimension
        cls_token = self.cls_token.expand(B, 1, E)
        goal_image_token = self.goal_image_token.expand(B, 1, E)

        # Adding classification and goal_img tokens to the tokens
        x = torch.cat((cls_token, obs_patches, goals_e, goal_image_token, patches_g), dim=1)
        ...
        if mask_:
            init_cls = 0
            init_obs0 = 1
            init_obs1 = 1 + obs_patches.size(1)
            init_texte = init_obs1
            init_texte1 = init_texte +goals_e.shape[1]
            init_goal_token = init_texte1
            init_goal = init_goal_token +1
            init_goal1 = init_goal + patches_g.size(1)
            mask = torch.ones((B, sequence_length, sequence_length), 
                              dtype=torch.bool, device=x.device)
            mask_texte = (torch.rand(B,device=x.device) < self._cfg.policy.text_mask_prob).float()

            for elem in range(B):
                if mask_texte[elem]==1:
                    mask[elem, :, init_texte: init_texte1] = 0
                    mask[elem, init_texte: init_texte1, :] = 0
                else:
                    mask[elem, :, init_goal_token: init_goal1] = 0
                    mask[elem, init_goal_token: init_goal1, :] = 0

                mask[elem,:,init_cls] = 1
                mask[elem,init_cls,:] = 1
        ...
        if targets is not None:
            if getattr(self._cfg.policy, 'action_loss_type', 'mse') == 'mse':
                loss = F.mse_loss(out, targets)    
            else:
                act_dim = self._cfg.env.action_dim*self._cfg.policy.action_stacking
                logits = out.view(B, act_dim,self.bins)
                loss  = F.cross_entropy(logits.reshape(-1,self.bins), targets.reshape(-1))
    ...
    def encode_text_goal(self, goal, tokenizer=None, text_model=None):
        ...
        if self._cfg.dataset.encode_with_t5:
            ...
            inputs1 = tokenizer(goal, return_tensors="pt",truncation=True,
                                max_length=self._cfg.max_block_size,
                                padding = "max_length")
            input_ids = inputs1.input_ids.to(self._cfg.device)
            attention_mask = inputs1.attention_mask.to(self._cfg.device)
            with _torch.no_grad():
                goal_emb = text_model.encoder(input_ids=input_ids,
